	...
	public RASServerSettings()
	{
		...
		try
		{
			...
			if(XmlFile.exists())
			{
				...
				ServerIpAdd = XmlNode.getNodeValue();//getTextContent();
				XmlNodeList = XmlDoc.getElementsByTagName("PORT");
				XmlNode = XmlNodeList.item(0);
				nServerPortNumber = Integer.parseInt(XmlNode.getNodeValue());
				XmlNodeList = XmlDoc.getElementsByTagName("TIMEOUT");
				XmlNode = XmlNodeList.item(0);
				ServerTimeOut = XmlNode.getNodeValue();//getTextContent();
			}
		}
