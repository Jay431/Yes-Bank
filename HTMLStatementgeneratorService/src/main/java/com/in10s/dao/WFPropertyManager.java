...
public class WFPropertyManager {
   ...
   private  Map<String,Float>  html5_compatable_browsers_map= null;

    public Map<String, Float> getHtml5_compatable_browsers_map() {
        return Collections.unmodifiableMap(this.html5_compatable_browsers_map);
    }
   ...
   private void buildPropertyManagerMaps() throws Exception{
        ...
        try{
            ...
            String htmlQuery = "SELECT BROWSER_NAME,MIN_SUPPORTED_VERSION FROM <<SCHEMA_NAME>>.HTML5_COMPATABLE_BROWSER_LIST";
            htmlQuery = htmlQuery.replace("<<SCHEMA_NAME>>", BoneCPDBPool.strSchemaName);
            ps = con.prepareStatement(htmlQuery);
            rs = ps.executeQuery();
            while (rs.next()) {
                html5_compatable_browsers_map.put(rs.getString(1),rs.getFloat(2));
            }
            if (ps != null) {
                ps.close();
                ps = null;
